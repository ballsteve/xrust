...
macro_rules! item_node_tests (
	...
	fn node_push_content() {
	    let mut d = $x();
	    let n = d.new_element(QName::from_local_name(NcName::try_from("Test").unwrap()))
	...
	fn item_node_type() {
	    assert_eq!(
		$y(QName::from_local_name(NcName::try_from("Test").unwrap()), Value::from("foobar")).node_type(),
		NodeType::Document
		)
	}
	...
	#[test]
	fn item_node_name() {
	    let d = $y(QName::from_local_name(NcName::try_from("Test").unwrap()), Value::from("foobar"));
	    match d.child_iter().nth(0) {
			Some(c) => {
		    	assert_eq!(c.node_type(), NodeType::Element);
		    	assert_eq!(c.name().map_or(String::from(""), |e| e.local_name().to_string()), "Test")
			}
	...
	fn item_node_pop() {
	    let mut d = $x();
	    let mut new = d.new_element(QName::from_local_name(NcName::try_from("Test").unwrap()))
		.expect("unable to create element node");
	    d.push(new.clone())
		.expect("unable to add node");
	    assert_eq!(d.to_xml(), "<Test></Test>");
	    ...
	    new.push(e.clone())
		.expect("unable to add node");
	    let mut f = d.new_element(QName::from_local_name(NcName::try_from("Bar").unwrap()))
		.expect("unable to create element node");
	    e.push(f)
		.expect("unable to add node");
	...
	fn item_node_parent() {
	    ...
	    let mut l1 = sd.new_element(QName::from_local_name(NcName::try_from("Level-1").unwrap()))
		.expect("unable to create element");
	    t.push(l1.clone())
		.expect("unable to append child");
	    let l2 = sd.new_element(QName::from_local_name(NcName::try_from("Level-2").unwrap()))
		.expect("unable to create element");
	    l1.push(l2.clone())
		.expect("unable to append child");
	    assert_eq!(l2.parent().unwrap().name().map(|e| e.local_name().to_string()),
	...
	fn item_node_ancestor() {
	    ...
	    t.push(l1.clone())
		.expect("unable to append child");
	    let mut l2 = sd.new_element(QName::from_local_name(NcName::try_from("Level-2").unwrap()))
		.expect("unable to create element");
	    l1.push(l2.clone())
		.expect("unable to append child");
	    ...
	    assert_eq!(aiter.next().unwrap().name().unwrap().local_name().to_string(), "Level-2");
	    assert_eq!(aiter.next().unwrap().name().unwrap().local_name().to_string(), "Level-1");
	    assert_eq!(aiter.next().unwrap().name().unwrap().local_name().to_string(), "Test");
	...
	fn item_node_children() {
	    ...
	    assert_eq!(citer.next().unwrap().name().unwrap().local_name().to_string(), "Level-1");
	    assert_eq!(citer.next().unwrap().name().unwrap().local_name().to_string(), "Level-2");
	...
	fn item_node_first_child() {
		...
		.expect("unable to append child");
	    match t.first_child() {
		Some(f) => assert_eq!(f.name().unwrap().local_name().to_string(), "Level-1"),
	...
	fn item_node_descend() {
	    ...
	    sd.push(t.clone())
		.expect("unable to append child");
	    let mut l1a = sd.new_element(QName::from_local_name(NcName::try_from("A").unwrap()))
		.expect("unable to create element");
	    t.push(l1a.clone())
		.expect("unable to append child");
	    let mut l1b = sd.new_element(QName::from_local_name(NcName::try_from("B").unwrap()))
		.expect("unable to create element");
	    t.push(l1b.clone())
		.expect("unable to append child");

	    let mut l2aa = sd.new_element(QName::from_local_name(NcName::try_from("A").unwrap()))
		.expect("unable to create element");
	    l1a.push(l2aa.clone())
		.expect("unable to append child");
	    l2aa.push(
	    ...
	    let mut l2ab = sd.new_element(QName::from_local_name(NcName::try_from("B").unwrap()))
		.expect("unable to create element");
	    l1a.push(l2ab.clone())
		.expect("unable to append child");
	    l2ab.push(
	    ...
	    let mut l2ba = sd.new_element(QName::from_local_name(NcName::try_from("A").unwrap()))
		.expect("unable to create element");
	    l1b.push(l2ba.clone())
		.expect("unable to append child");
	    l2ba.push(
	    ...
	    let mut l2bb = sd.new_element(QName::from_local_name(NcName::try_from("B").unwrap()))
		.expect("unable to create element");
	    l1b.push(l2bb.clone())
		.expect("unable to append child");
	    l2bb.push(
	    ...
	    assert_eq!(diter.next().unwrap().name().unwrap().local_name().to_string(), "A");
	    assert_eq!(diter.next().unwrap().name().unwrap().local_name().to_string(), "A");
	    assert_eq!(diter.next().unwrap().value().to_string(), "AA");
	    assert_eq!(diter.next().unwrap().name().unwrap().local_name().to_string(), "B");
	...
	fn item_node_next() {
	    ...
	    assert_eq!(niter.next().unwrap().name().unwrap().local_name().to_string(), "Level-2");
	...
	fn item_node_prev() {
	    ...
	    assert_eq!(piter.next().unwrap().name().unwrap().local_name().to_string(), "Level-2");
	    assert_eq!(piter.next().unwrap().name().unwrap().local_name().to_string(), "Level-1");
	    match piter.next() {
		None => {},
		_ => panic!("iterator should have no more items")
	    }
	...
	fn item_node_attr() {
	    ...
	    let v = aiter.next().unwrap().name().unwrap().local_name().to_string();
	    if v == "role" {
		assert_eq!(aiter.next().unwrap().name().unwrap().local_name().to_string(), "phase");
	    } else if v == "phase" {
		assert_eq!(aiter.next().unwrap().name().unwrap().local_name().to_string(), "role");
	    } else {
	...
	fn item_node_shallow_copy_element() {
		...
		let sd = $z();
		let b1: Vec<RNode> = sd.descend_iter().filter(|n| n.get_attribute(&QName::from_local_name(NcName::try_from("id").unwrap())).to_string() == String::from("b1")).collect();
		let b9: Vec<RNode> = sd.descend_iter().filter(|n| n.get_attribute(&QName::from_local_name(NcName::try_from("id").unwrap())).to_string() == String::from("b9")).collect();
		assert_eq!(b1[0].cmp_document_order(&b9[0]), Ordering::Less)
	}
		#[test]
		fn item_node_cmp_doc_order_2() {
		let sd = $z();
		let b10: Vec<RNode> = sd.descend_iter().filter(|n| n.get_attribute(&QName::from_local_name(NcName::try_from("id").unwrap())).to_string() == String::from("b10")).collect();
		let b6: Vec<RNode> = sd.descend_iter().filter(|n| n.get_attribute(&QName::from_local_name(NcName::try_from("id").unwrap())).to_string() == String::from("b6")).collect();
	...
	fn item_node_partialeq_1_pos() {
	    ...
	    let mut od = $x();
	    let mut u = od.new_element(QName::from_local_name(NcName::try_from("Test").unwrap()))
		.expect("unable to create element");
	    od.push(u.clone())
		.expect("unable to append child");
