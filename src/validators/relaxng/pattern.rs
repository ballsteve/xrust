...
pub(crate) fn patternmaker(n: RNode) -> Result<Pattern, PatternError<'static>> {
    //let _ =
    match n.node_type() {
        NodeType::Document => patternmaker(n.child_iter().next().unwrap()),
        NodeType::Element => {
            if n.name().get_nsuri() != Some("http://relaxng.org/ns/structure/1.0".to_string()) {
                ...
                match n.name().get_localname().as_str() {
                    ...
                    "element" => patternmaker_element(n),
                    _ => Ok(Pattern::Empty),
                }
            }
        ...
        NodeType::Text => Err(PatternError::NotRelaxNG),
        NodeType::Attribute => Err(PatternError::Other("not yet implemented")),
        NodeType::Comment => Err(PatternError::Other("not yet implemented")),
        NodeType::ProcessingInstruction => Err(PatternError::Other("not yet implemented")),
        NodeType::Reference => Err(PatternError::Other("not yet implemented")),
        NodeType::Unknown => Err(PatternError::Other("not yet implemented")),
    }
    //;
    //Ok((Pattern::Empty))
}
...
fn patternmaker_element(n: RNode) -> Result<Pattern, PatternError<'static>> {
    let nsattr = n
        .get_attribute(&QualifiedName::new(None, None, String::from("ns")))
        .to_string();
    ...
    let mut name = n
        .get_attribute(&QualifiedName::new(None, None, String::from("name")))
        .to_string();
    let mut res = vec![];
    ...
    match children.next() {
        ...
        Some(rn) => {
            match (
                rn.name().get_nsuri().unwrap_or("".to_string()).as_str(),
                rn.name().get_localname().as_str(),
            ) {
                ("http://relaxng.org/ns/structure/1.0", "name") => {
                    name = rn.to_string();
                }
                ("http://relaxng.org/ns/structure/1.0", _) => match patternmaker(rn) {
                    Err(PatternError::NotRelaxNG) => {}
                    Err(e) => return Err(e),
                    Ok(pat) => res.push(pat),
                },
    ...
    match res.len() {
        ...
        _ => p = patternmaker_group(res),
    };
    match p {
        Ok(p1) => Ok(Pattern::Element(
            NameClass::Name(namespace, name),
            Box::new(p1),
        )),
        Err(e) => Err(e),
    }
...
fn patternmaker_group(nodes: Vec<Pattern>) -> Result<Pattern, PatternError<'static>> {
    if nodes.len() > 2 {
        ...
        match patternmaker_group(nodesi.collect()) {
            Ok(rest) => Ok(Pattern::Group(Box::new(first), Box::new(rest))),
            Err(e) => Err(e),
        }
    } else {
        Ok(Pattern::Group(
