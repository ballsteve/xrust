...
use crate::transform::{Transform, NodeMatch, Axis};
...
pub(crate) fn context<N: Node>(ctxt: &Context<N>) -> Result<Sequence<N>, Error> {
    ctxt.cur.get(ctxt.i).map_or(
        Err(Error::new(ErrorKind::DynamicAbsent, String::from("no context"))),
        |i| Ok(vec![i.clone()])
    )
}
