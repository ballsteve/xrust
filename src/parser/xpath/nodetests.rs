...
-> Box<dyn Fn(ParseInput<N>) -> Result<(ParseInput<N>, NodeTest), ParseError> + 'a> {
    Box::new(map_with_state(
        tuple3(ncname::<N>(), tag(":"), ncname()),
        |(prefix, _, localpart), ss| {
            if let Some(n) = ss.cur {
                if let Some(nsuri) = n
                    .namespace_iter()
                    .find(|ns| prefix == ns.name().to_string())
                {
                    NodeTest::Name(NameTest {
                        ns: Some(WildcardOrName::Name(nsuri.value())),
                        prefix: Some(Rc::new(Value::from(prefix))),
                        name: Some(WildcardOrName::Name(Rc::new(Value::from(localpart)))),
                    })
                } else {
                    panic!("no namespace declaration for prefix \"{}\"", prefix)
                }
            } else {
                NodeTest::Name(NameTest {
                    ns: None,
