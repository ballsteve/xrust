...
impl<'a> Item<'a> for Sequence<'a> {
    ...
    fn doc(&self) -> Result<Box<dyn Item<'a> + 'a>, Error> {
      Result::Err(Error{kind: ErrorKind::Unknown, message: String::from("sequences don't have a document")})
    }
    fn children(&self) -> Result<Vec<Box<dyn Item<'a> + 'a>>, Error> {
      Result::Err(Error{kind: ErrorKind::Unknown, message: String::from("sequences don't have children")})
    }
    fn parent(&self) -> Result<Box<dyn Item<'a> + 'a>, Error> {
      Result::Err(Error{kind: ErrorKind::Unknown, message: String::from("sequences don't have a parent")})
    }
}
...
mod tests {
    ...
    #[test]
    fn ebv_int_0() {
        let s: Sequence = vec![Box::new(Value::Integer(0))];
	assert_eq!(s.to_bool(), false)
    }
    #[test]
    fn ebv_int_1() {
        let s: Sequence = vec![Box::new(Value::Integer(10))];
	assert_eq!(s.to_bool(), true)
    }
    #[test]
    fn ebv_double_0() {
        let s: Sequence = vec![Box::new(Value::Double(0.0))];
	assert_eq!(s.to_bool(), false)
    }
    #[test]
    fn ebv_double_1() {
        let s: Sequence = vec![Box::new(Value::Double(0.01))];
	assert_eq!(s.to_bool(), true)
    }
    #[test]
    fn ebv_string_0() {
        let s: Sequence = vec![Box::new(Value::String(String::from("")))];
	assert_eq!(s.to_bool(), false)
    }
    #[test]
    fn ebv_string_1() {
        let s: Sequence = vec![Box::new(Value::String(String::from("false")))];
	assert_eq!(s.to_bool(), true)
    }
}
