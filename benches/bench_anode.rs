...
fn make_anode(n: u64) -> RANode {
    ...
    (1..n).for_each(|i| {
        ...
        (1..n).for_each(|k| {
            ...
            l1.push(l2).expect("unable to append level-2 element");
        });
        b.push(l1).expect("unable to append level-1 element");
    });
    let mut a = Rc::new(ANodeBuilder::new(NodeType::Document).build());
    a.push(b).expect("unable to append toplevel element");
    a
}

fn anode(c: &mut Criterion) {
    c.bench_function("anode 100", |b| b.iter(|| make_anode(black_box(100))));
}
